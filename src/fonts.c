...
struct Font *init_fonts() {
  ...
  struct Font fonts_arr[] = {
      {"JetBrains", "JetBrainsMonoNFM", "Bold", 14, 1.0},
      {"Victor", "VictorMono NFP", "Bold", 18, 1.0},
      {"Lekton", "Lekton Nerd Font Mono", "Bold", 21, 0.9},
  };
...
int main(int argc, char **argv) {
  ...
  // temp file
  snprintf(temp_filename, FILENAME_MAX, "%s/.cache/temp_font", getenv("HOME"));
  ...
  if (access(st_config_filename, F_OK) != 0) {
    printf("ST configuration does not exist at $HOME/.config/st/config.h!\n");
    exit(-1);
  }
  ...
  while (true) {
    ...
    // putting the contents of the file with the modified font in a temp file
    if (temp_file == NULL) {
